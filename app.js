...
//Thought / Goal.2  Nested For loops. First one is to sort (and hopefully convert)
//Swich or if statement? 

//for loop 1. The outside one. Needs to just count. 

//STEP ONE defining things
//potental options 
/*
let penny = 'penny'
...
// {
           //     console.log(`There are ${PennyCount} pennies! ` + `That equals to $ ${(PennyCount * .01).toFixed(2)}`)
          //  }
/*

for (coin = 0 ; coin < challengeData.length ; coin ++)
    {
        console.log(`There is ${challengeData.length} coins`) //i like. keep. 
            //of the 467 coins there are... 
    }
        if ('penny') 
            {
            console.log(`There are ${PennyCount} pennies! ` + `That equals to $ ${(PennyCount * .01).toFixed(2)}`)
            } else if ('dime'){
            console.log(`There are ${DimeCount} dimes! ` + `That equals to $ ${(DimeCount * .10).toFixed(2)}`)
           } else if ('nickel')
           {
            console.log(`There are ${NickelCount} dimes! ` + `That equals to $ ${(NickelCount * .05).toFixed(2)}`)
           }

*/

/*
my potential nested for loop outside. / goal adn desire

for (coin = 0 ; coin < challengeData.length ; coin ++){
    console.log ( There are &{ this many} coins )

        for (pen = 0; pen < challenge.Data.lenght ; pen++)
